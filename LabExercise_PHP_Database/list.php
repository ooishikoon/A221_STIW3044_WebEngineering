...
<h2 style='text-align: center;'><b>Student Data Info List</b></h2>
 ...
 while ($row = mysqli_fetch_assoc($result)) {
     ...
     echo "<img src='data:image/jpg;base64,".base64_encode( $row['std_image'] )."'
        style='object-fit:contain;
         ...
         <td>" . $row["race"] . "</td>
         <td>" . $row["gender"] . "</td>
         <td><button type='submit' class='btn btn-outline-primary btn-sm' name='edit' value='" . $row["id"] . "'>Edit</button></td></tr>";
        }
 echo "</table></form>";
?>
...
<body>
    <form action="list.php" method="post">
        ...
        Column: <select name="option" id="option">
        ...
        if(isset($_POST["submit"])){
            ...
            $sqlstudent  = "SELECT * FROM tbl_student WHERE $option = '$search'"; 
            ...
            while($row = $result->fetch_assoc() ){
                echo "<tr><tr>
                <td><img src='data:image/jpg;base64,".base64_encode( $row['std_image'] )."'
                style='object-fit:contain;
                    width:100px;
                    height:100px;
                    border: solid 1px #CCC'/></td>
